...
def _portfolio_html(seen_data):
    ...
    # Live P&L JavaScript — fetches prices every 60s via Yahoo Finance (free, no key)
    # Uses Yahoo v8/chart API: no credits, no limits, works from browser
    live_pnl_js = f"""
<script>
(function() {{
  ...
  function yahooTicker(t) {{
    var eu = {{'IWDA':'IWDA.AS','INRG':'INRG.AS','IGLN':'IGLN.AS'}};
    return eu[t] || t;
  }}
  function fetchPrice(ticker) {{
    var yt = yahooTicker(ticker);
    var url = 'https://query1.finance.yahoo.com/v8/finance/chart/' + yt + '?interval=1m&range=1d';
